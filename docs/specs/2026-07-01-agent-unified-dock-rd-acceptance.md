...
2. AgentGUI `@` agent 候选从 `workspace-app` 伪应用迁移到 `agent-target`:
   ...
   - AgentGUI composer host capabilities 包含 `agent-target`。
   - AgentGUI Apps tab 仅查询 `workspace-app`；Agents tab 查询
     `agent-target`。
...
| agent-target @ provider migration       | PASS | 新增 renderer/external-core/ui-rich-text/AgentGUI tests 覆盖 `agent-target` provider、workspace-app 排除 agent pseudo apps、默认 external providers 包含 agent-target、显式 workspace-app 查询只返 apps、AgentGUI host capabilities 包含 agent-target、Agents tab 查询 agent-target、Apps tab 不混入 agents、colon href 保持。 |
...
| changed-aware validation                | RISK | `pnpm check:changed` 通过 19/20 lanes；AgentGUI 大 vitest lane 暴露的旧 tab 顺序断言已修复并 focused 通过。`pnpm check:changed -- --failed-only` 随后执行到 13/14 files、558 tests passed 后因 Node/Vitest worker OOM 退出，未再出现业务断言失败。                                                                             |
| full repository check                   | PASS | pre-push `pnpm check:full` 已通过并完成远端 push。                                                                                                                                                                                                                                                                             |
...
```sh
pnpm --dir packages/agent/gui exec vitest run --environment jsdom \
  agent-gui/agentGuiNode/AgentMentionSearchController.spec.ts \
  agent-gui/agentGuiNode/AgentMentionLabels.spec.ts \
  agent-gui/agentGuiNode/agentRichText/agentFileMentionExtension.spec.ts \
  --reporter verbose

cd apps/desktop && node --import ./test/register-asset-stub.mjs --test \
  --experimental-strip-types \
  ./src/renderer/src/features/workspace-agent/services/createDesktopAgentGUIWorkbenchHostInput.test.ts

pnpm --dir packages/agent/gui exec vitest run --environment jsdom \
  agent-gui/agentGuiNode/AgentGUINode.spec.tsx \
  -t "cycles mention tabs|keeps arrow navigation inside browse results" \
  --reporter verbose
...
lane initially found focused test assertions in AgentGUI specs; each product
assertion was fixed and passed standalone. Re-running the large AgentGUI lane
then failed in the test worker with heap OOM or hung under larger heap, with no
remaining product assertion failure in the focused regressions.
