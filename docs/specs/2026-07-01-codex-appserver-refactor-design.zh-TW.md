...
| #         | 決策                                                                                                                                                                                                                                                                        | 理由                                                                                                                                                                                                                                                                            |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1        | **只退場 _Codex-over-ACP_ 路徑（`codex_adapter.go`）。** app-server 是 Codex 唯一未來路徑。**通用 ACP 棧保留**——其他 agent 透過它接入。                                                                                                                                     | 產品方向。`standard_acp_adapter.go`（Hermes / OpenClaw / 未來 agent）與共用 ACP 基礎設施留著；只有 Codex 對 ACP 的使用退場。                                                                                                                                                    |
...
| 檔案                                                                   | 處置               | 原因                                                                                                                                                                              |
| ---------------------------------------------------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `standard_acp_adapter.go`（3503）                                      | **保留**           | 通用 ACP adapter，服務 Hermes / OpenClaw / 未來 agent（`NewHermesAdapter`、`NewOpenClawAdapter`…）。可複用路徑。per-agent 差異已用 `standardACPConfig` 參數化，不是新開 adapter。 |
...
Transport ──→ typed Client ──→ Thread ┤
 (stdio)      (pending req      registry ├─ Approval Resolver ──→ durable pending state + typed responder
       ...
       Codex-over-ACP adapter（codex_adapter.go）           Hermes / OpenClaw / 未來 agent）
       — legacy，Step 8 刪除                                 — 保留
