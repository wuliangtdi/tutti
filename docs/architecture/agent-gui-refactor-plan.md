...
provider 切片（依赖 P1 的能力描述符 schema）：🚧 通用描述符类型与消费适配层已经落地。✅ Codex、Claude Code 与 OpenCode 已完成单 provider 原子收口：runtime/status/installer/auth watcher、composer/model/skill/capability/slash catalog、turn lifecycle 投影、系统 target、事件归一化均由 descriptor 驱动；Claude Code 只保留 SDK 边车运行时，不存在 ACP 注册；OpenCode 的标准 ACP command、permission mode 映射和 spawn-time model env 由通用 ACP descriptor strategy 构造。slash policy 与 composer 展示策略通过显式契约下发，GUI 的 plan/权限/skill/slash/model-refresh/model image gating 行为只读能力、目录、invocation 与 behavior，不再按 provider 名称猜测；identity/target/icon key 生成物及漂移检查覆盖三个已迁移 provider，并同时校验全语言 locale key 与完整图标资源，未知图标不再静默回退。其余 provider 的迁移、注册表与全量枚举最终相等校验，以及未迁移 provider 的 legacy fallback 清理尚未完成。
