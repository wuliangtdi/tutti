...
先说健康的部分：标准 ACP 适配层不需要重构。`standard_acp_adapter.go` 是共享引擎，每个走 ACP 协议的 provider 只是一份 86 到 160 行的声明式配置（如 `acp_provider_opencode.go` 仅 86 行）。Claude Code 的 SDK-only 运行时边界与内部职责见 [Claude Code SDK Runtime](./claude-code-sdk-runtime.md)。需要修的是适配层之外的两处。
...
provider 切片（依赖 P1 的能力描述符 schema）：🚧 通用描述符类型与消费适配层已经落地。✅ Codex 与 Claude Code 已分别完成单 provider 原子收口：runtime/status/installer/auth watcher、model/skill/capability/slash catalog、turn lifecycle 投影、系统 target、事件归一化均由 descriptor 驱动；Claude Code 只保留 SDK 边车运行时，不存在 ACP 注册。slash policy 与 composer 展示策略通过 OpenAPI 显式契约下发，GUI 的 plan/权限/skill/slash/model-refresh 行为只读能力、目录、invocation 与 behavior，不再按 Codex 或 Claude Code 名称猜测；identity/target/icon key 生成物及漂移检查已接入，检查同时覆盖全语言 locale key 与完整图标资源，未知图标不再静默回退。其余 provider 的迁移、注册表与全量枚举最终相等校验，以及未迁移 provider 的 legacy fallback 清理尚未完成。
...
实现状态：🚧 提交子域已迁移：`pendingIntents` 是提交乐观消息、接受、确认、
结果未知、失败与到期的单一事实源；controller 不再把命令完成还原为 Promise
工作流，桌面 `sendInput` 乐观快照与回滚桥及 conversation submit-pending
双轨状态已删除。会话激活的 pending record、`session/activate` 命令、超时与
canonical session 确认 reducer 已落地并有事件交错测试，但 controller 的新建/
重连激活链路与剩余 session-view/list 覆盖层尚未原子切换，因此切片 2 尚未完成。
