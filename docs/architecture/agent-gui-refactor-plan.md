...
保持低于 800 行硬限制并只负责纵向模块装配。终值基线与完整验证
...
会话 Rail 的滚动、section 折叠和“显示更多”属于上述组件局部 UI 状态，按
`workspaceId + agentTargetId/all` 记忆；搜索 query 只建立瞬时导航状态，不扩张
永久 scope map。query controller 只报告当前请求 scope 是否已精确 resolved，不
拥有 DOM 或滚动位置。`activeConversationId` 只表达选中事实；外部打开与新建会话
通过显式 reveal intent 请求滚动，Rail 点击与 provider 恢复不发滚动命令。
...
低于 800 行的薄 controller 已落地。切片 7 的镜像、过渡导出、旧事件与终值基线清理已完成；
...
对外发布 engine 会话摘要与 daemon membership 组成的单一不可变快照；snapshot
本身就是 committed value，受影响页面返回前保持不变，全部返回后从最新 engine
state 与 resolved membership 重建一次。Rail 不读取 mutation history，view 不再
单独订阅 live engine entity 或保存 stale section。
...
就绪门槛模块和渲染预算已落地；controller 保持低于 800 行且只负责模块装配，view 783 行。
...
- Rail 视图状态：按 target/all 记忆的 scrollTop、section 折叠和可见数量，以及外部打开/新建的瞬时 reveal intent，归会话列表模块；搜索 query 不建立永久视图 scope；
