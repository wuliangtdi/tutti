...
5. 历史/runtime 层保留 `nexight` provider identity 兼容旧数据；桌面新入口层下线旧的 `agent-nexight`/"Tutti" 入口，改由 `agent-tutti-agent` / `local:tutti-agent` 承担。
...
| 标准 ACP           | `gemini`、`hermes`、`openclaw`、`nexight`（部分） | `standardACPAdapter` + 每 provider 一份 `standardACPConfig`                                                                              | 已参数化，是本方案 adapter 参数化的范式参照                                                                                                                              |
...
对比范式：`standard_acp_adapter.go` 的 `standardACPConfig`（字段含 `provider`、`adapterName`、`command`、`commandResolver`、`env`、`initializeParams`、`authRequiredMessage`、`permissionModeID` 等），一个 adapter 类型被 `NewNexightAdapter…` / `NewGeminiAdapter…` / `NewHermesAdapter…` / `NewOpenClawAdapter…` 用不同配置实例化。app-server 侧目前没有对应物——这正是本方案的核心改造。
...
- `packages/agent/gui/workbench/providerCatalog.ts`：`agentGuiWorkbenchDefaultDockProviders = ["codex","claude-code"]`、`dockSuppressedProviders = ["hermes","gemini"]`、`comingSoonProviders = ["nexight"]`、brand label Record（带 `// i18n-check-ignore` 注释）。desktop 侧经 `workspaceAgentProviderCatalog.ts` shim 转 re-export。
...
- Launchpad：`WorkspaceLaunchpadOverlay.tsx` 遍历 `workspaceAgentGuiProviders`；旧 `nexight` 的 "Tutti" 特判已下线，新入口统一使用 `tutti-agent` / "Tutti Agent"。
- 图标三处注册表：
...
宿主登录态 cookie session_id
  ...
  -> 登出：tutti-agent logout（调 revoke 并清理 auth.json）
```

关键约束：

- refresh token 会 rotate，旧 token 重放会导致**整个 token family 被撤销**——多个并发 agent home 共享同一 refresh family 是危险的，token 处理需要单一属主或串行化。
- 当前线上策略（AppConfig `tsh_llm_access_policy`）：Tutti desktop app id 仅允许 `gpt-5.4`、协议 `openai_responses`、provider `openrouter`；其他模型返回 403。**因此模型列表必须来自 app-server `model/list` 实时结果，不能做静态兜底列表**。
...
- `services/tuttid/biz/agentprovider/provider.go`：加常量、进 `All()`、`Normalize` 加 case。**不得动现有 `"tutti"` 别名**（它归一化到 `nexight`，改动会静默改变既有行为）。
...
tuttid 获取宿主登录态（account/session 集成点，见开放问题）
  -> POST /api/account/auth/v1/llm-token（requested_app_id "tutti"）
  -> stdin JSON -> spawn `tutti-agent login --with-tutti-llm-tokens`
  -> 校验 TUTTI_AGENT_HOME/auth.json 的 tutti_llm 可用
  -> 启动 app-server
```

登出/清理：宿主登出时执行 `tutti-agent logout`（revoke + 清 auth.json），或 daemon 直接调 revoke 后清理 home。
...
- **Nexight/Tutti 命名决策**：旧 `nexight` 只保留历史/runtime identity，不再作为桌面新入口；新建会话、dock、launchpad、mention 和 composer/rail 入口统一走 `tutti-agent` / "Tutti Agent"。
...
```text
daemon 获取宿主登录态 -> account llm-token 签发（app_id tutti）
  ...
  -> 登出 -> tutti-agent logout / revoke -> family 失效
```
...
- `services/tuttid/biz/agentprovider/provider.go`：常量、`All`、`Normalize`（保留 `tutti`→nexight）。
...
- launchpad "Tutti"（nexight）与 "Tutti Agent" 展示名冲突的产品决策落地。
...
- **展示名冲突**：已决策下线桌面新入口里的旧 Nexight/"Tutti" 伪应用；保留 Nexight 仅用于历史/runtime provider identity 兼容。
- 不改 `tutti`→`nexight` 既有别名。
