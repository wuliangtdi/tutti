...
- Evidence: Runtime policy already routes `mention://workspace-reference/<id>?source=...&workspaceId=...` to the `reference` skill, but Claude Code's injected first-tool-call routing only mapped `workspace-issue`, `workspace-app`, and `agent-session`.
...
- Fix: Map `workspace-reference` mentions to the `reference` skill in Claude Code mention routing and add a regression test.
- Verification:
