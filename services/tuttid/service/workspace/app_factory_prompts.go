...
func writeAppFactoryMentionContext(workspace workspacebiz.Summary, physicalRoot string, job workspacebiz.AppFactoryJob) error {
	...
	payload := appFactoryMentionContext{
		...
		Constraints: []string{
			...
			"If the app needs local agent or local LLM execution, Codex, Claude, or app-owned MCP/tooling, follow the tutti-agent-workspace-app skill and keep provider detection and run execution server-owned instead of using TUTTI_CLI agent/codex/session polling.",
