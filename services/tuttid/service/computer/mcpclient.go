// Package computer hosts a daemon-owned computer session driven by
// cua-driver. The daemon is the MCP client and exposes computer actions to
// agents through the `tutti computer` CLI.
package computer
...
func (c *mcpClient) readLoop() {
	...
	for {
		...
		if frame.ExitCode != nil {
			c.fail(fmt.Errorf("computer MCP process exited (code %d)%s", *frame.ExitCode, c.stderrTail()))
			return
		}
...
func (c *mcpClient) respond(id json.RawMessage, result any, rpcErr *rpcError) {
	...
	if err := c.send(payload); err != nil {
		c.fail(fmt.Errorf("send computer MCP response: %w", err))
	}
}
...
func (c *mcpClient) call(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	if c.closed {
		...
		c.mu.Unlock()
		if err == nil {
			err = errors.New("computer MCP client closed")
		}
		return nil, err
	}
	c.nextID++
