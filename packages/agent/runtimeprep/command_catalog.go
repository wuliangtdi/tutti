...
func commandGuideFromCapabilities(cliName string, capabilities []CommandCapability) string {
	cliName = normalizeCLICommandName(cliName)
...
func runtimeCommandFromCapability(cliName string, capability CommandCapability) (runtimeCommand, bool) {
	...
	if capability.Source.Kind == CommandSourceApp && strings.TrimSpace(capability.Source.AppName) != "" {
		if description != "" {
			description += " "
		}
		description += "Provided by workspace app " + strings.TrimSpace(capability.Source.AppName) + "."
	}
	if capability.Source.Kind == CommandSourceApp && strings.TrimSpace(capability.Source.AppID) != "" {
		if description != "" {
			description += " "
		}
...
func appCapabilityIsOpenCommand(capability CommandCapability) bool {
	...
	return capability.Source.Kind == CommandSourceApp &&
		len(path) > 0 &&
